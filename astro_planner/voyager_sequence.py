...
#         self.filter_sequence = filter_sequence
        ...
        self.item_template = '<item id="ref-{ref_id}" xsi:type="SOAP-ENC:string">{value}</item>'
        ...
        self.el_refs = [self.gain_refs[-1] +
                        1 + i for i in range(self.n_slots)]
    ...
    def _update_lines(self):
        ...
        data_dir = '{}\{}'.format(DEFAULT_DATA_PATH, target_name)

        profile_filename = '{}\{}.{}'.format(
            DEFAULT_PROFILE_PATH, self.profile.name, VOYAGER_PROFILE_EXTENSION)

        value_dict = {RA_KEY: self.target.ra_string,
    ...
    def add_filters(self):
        ...
        for slot_number, (filter_name, data) in enumerate(self.filter_sequence.items()):
            id_ref += 1
            string = self._gain_offset(
                self.gain_refs[slot_number], slot_number + 1)
            filter_list.append(string)

        for slot_number, (filter_name, data) in enumerate(self.filter_sequence.items()):
            id_ref += 1
            string = self._seq_element(self.el_refs[slot_number], self.sequence.profile.sensor.filter_wheel.filter_pos[filter_name],
                                       filter_name, data['exposure'], data['binning'], data['n_subs'])
            filter_list.append(string)
    ...
    def write_file(self, file_out='test.s2q'):
        ...
        lines_out = [line.replace(
            '\n', '') for line in self.sequence_lines if '</SOAP-ENV' not in line]
